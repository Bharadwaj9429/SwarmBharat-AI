...
class SwarmBharatCore:
    ...
    def _should_fetch_data(self, query: str, domain: str) -> bool:
        ...
        data_keywords = {
            'career': ['job', 'career', 'salary', 'resume', 'interview', 'work', 'employment', 'software', 'engineer', 'data science', 'developer'],
            'finance': ['invest', 'money', 'finance', 'mutual fund', 'fd', 'stock', 'crypto', 'tax', 'income tax', 'gst', 'savings', 'gold price'],
            'immigration': ['immigration', 'visa', 'pr', 'express entry', 'canada', 'australia', 'germany', 'usa', 'uk', 'crs', 'ielts'],
            'farming': ['farming', 'crop', 'agriculture', 'scheme', 'kisan', 'pm-kisan', 'pm kisan', 'kcc', 'kisan credit', 'soil health', 'fpo', 'mandi', 'msp', 'farmer', 'land', 'harvest', 'tractor'],
            'health': ['health', 'hospital', 'medicine', 'insurance', 'doctor', 'ayushman', 'aarogyasri', 'treatment', 'disease'],
            'legal': ['legal', 'court', 'lawyer', 'case', 'rights', 'passport', 'document', 'visa', 'rti', 'consumer'],
            'business': ['business', 'gst', 'startup', 'company', 'entrepreneur', 'udyam', 'msme', 'registration'],
            'education': ['education', 'college', 'school', 'scholarship', 'loan', 'jee', 'neet', 'upsc', 'course', 'degree'],
            'government': ['government', 'scheme', 'benefit', 'subsidy', 'pm-', 'yojana', 'portal']
        }
        ...
        domain_patterns = {
            ...
            "farming": {
                "patterns": [
                    ...
                    "pm-kisan", "kisan", "pm kisan", "kcc", "kisan credit card",
                    "soil health card", "fpo", "farmer producer", "crop insurance",
                    "msp", "mandi", "agricultural", "agri", "rythu bandhu",
                    "fasal bima", "pmfby", "farm equipment", "tractor subsidy"
                ],
                "weight": 2.5,
    ...
    async def _fetch_real_data(self, domain: str, query: str) -> Dict[str, Any]:
        ...
        if domain == "immigration":
            ...
            # Fetch farming schemes and benefits with fallback
            try:
                # Static fallback data for PM-KISAN and other schemes
                real_data["schemes"] = {
                    "pm_kisan": {
                        "name": "PM-KISAN (Pradhan Mantri Kisan Samman Nidhi)",
                        "amount": "₹6,000 per year",
                        "frequency": "₹2,000 every 4 months",
                        "eligibility": "Small and marginal farmers with land",
                    ...
                    "kcc": {
                        ...
                        "benefits": "Crop loan up to ₹3 lakh at 7% interest (with subsidy)",
                        "eligibility": "All farmers, sharecroppers, tenant farmers",
                        "features": "No collateral needed up to ₹1.6 lakh"
                    },
                    "soil_health": {
                        "name": "Soil Health Card Scheme",
                        "benefits": "Free soil testing and crop recommendations",
                        "eligibility": "All farmers"
                    },
                    "fpo": {
                        "name": "Farmer Producer Organizations (FPO)",
                        "benefits": "Collective bargaining, better market access",
                        "support": "Up to ₹18 lakh matching grant"
                    }
                }
                
                # Add state-specific schemes if we know the user's state
                user_state = personal.get("state", "Telangana")
                if user_state:
                    real_data["state_schemes"] = {
                        "state": user_state,
                        "schemes": [
                            f"Rythu Bandhu ({user_state}) - ₹10,000 per acre per year",
                            f"Crop Insurance ({user_state}) - Pradhan Mantri Fasal Bima Yojana",
                            "Free power for agriculture pumps"
                        ]
                    }
                
                logger.info(f"✓ Loaded farming schemes data for state: {user_state}")
                
            except Exception as e:
                logger.warning(f"Failed to fetch farming data: {e}")
        
        elif domain == "education":
            # Fetch education schemes and scholarships
            try:
                real_data["scholarships"] = {
                    "national": [
                        {
                            "name": "National Scholarship Portal (NSP)",
                            "eligibility": "Students from SC/ST/OBC/Minority communities",
                            "benefits": "Up to ₹50,000 per year for tuition + maintenance",
                            "website": "scholarships.gov.in"
                        },
                        {
                            "name": "Pragati Scholarship (AICTE)",
                            "eligibility": "Girl students in technical degree programs",
                            "benefits": "₹50,000 per year + tuition fee"
                        },
                        {
                            "name": "Fulbright-Nehru Fellowship",
                            "eligibility": "Outstanding students for Master's/PhD in USA",
                            "benefits": "Full funding for US education"
                        }
                    ],
                    "state": [
                        "State-specific scholarships based on user's location"
                    ]
                }
                
                # Add education loan info
                real_data["education_loans"] = {
                    "vidya_lakshmi": {
                        "name": "Vidya Lakshmi Portal",
                        "benefits": "Compare and apply for education loans from multiple banks",
                        "max_amount": "Up to ₹75 lakh for overseas education",
                        "interest": "8.5% - 11% depending on bank",
                        "website": "vidyalakshmi.co.in"
                    }
                }
                
                logger.info("✓ Loaded education schemes data")
                
            except Exception as e:
                logger.warning(f"Failed to fetch education data: {e}")
