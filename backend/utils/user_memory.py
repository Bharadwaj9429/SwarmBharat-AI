...
# Import MongoDB client for fast storage
...
class UserMemory:
    ...
    def __init__(self, user_id: str, storage_path: str = "data/users"):
        ...
        # Create directory if needed (fallback)
        os.makedirs(storage_path, exist_ok=True)
        
        # Load or initialize (file-based for now, async MongoDB load later)
        self.profile = self._load_profile()
    
    def _load_profile(self) -> Dict[str, Any]:
        """Load user profile from file (fast sync load)"""
        # File-based for fast sync initialization
    ...
    async def _save_profile(self) -> bool:
        """Persist profile to file (fast sync write) and optionally MongoDB"""
        try:
            self.profile["updated_at"] = datetime.now().isoformat()
            
            # Fast file write (synchronous for speed)
            with open(self.file_path, 'w', encoding='utf-8') as f:
                json.dump(self.profile, f, indent=2, ensure_ascii=False)
            
            # Optional: Async MongoDB backup (non-blocking)
            if mongodb_client.connected and mongodb_client.db:
                try:
                    await mongodb_client.save_user_profile(self.user_id, self.profile)
                except Exception as e:
                    logger.debug(f"MongoDB backup failed (non-critical): {str(e)}")
